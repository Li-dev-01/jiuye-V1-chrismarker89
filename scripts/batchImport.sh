...
# 批量导入测试数据到Cloudflare D1数据库
# 使用方法: bash scripts/batchImport.sh [database-name]

set -e  # 遇到错误立即退出

# 默认数据库名称
DATABASE_NAME=${1:-"employment-survey-db"}
SQL_DIR="generated-data/sql"

echo "🗄️ 开始批量导入测试数据到数据库: $DATABASE_NAME"
...
if [ ! -d "$SQL_DIR" ]; then
    ...
    echo "💡 请先运行: node scripts/importToDatabase.cjs"
    exit 1
fi

# 检查是否有SQL文件
if [ ! -f "$SQL_DIR/01-cleanup.sql" ]; then
    echo "❌ 找不到SQL文件，请先生成SQL文件"
    echo "💡 请先运行: node scripts/importToDatabase.cjs"
    exit 1
fi
...
echo ""
echo "🧹 清理测试数据命令 (如需要):"
echo "   wrangler d1 execute $DATABASE_NAME --file=generated-data/sql/01-cleanup.sql"
