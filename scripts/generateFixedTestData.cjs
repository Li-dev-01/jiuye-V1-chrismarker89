...
/**
 * 修正版测试数据生成器
 * 解决universal_questionnaire_responses表的user_id类型不匹配问题
 */
...
class QuestionnaireGenerator {
  generateUniversalResponse(index) {
    ...
    const responseData = {
      ...
      // 添加用户标识信息到JSON中，而不是依赖外键
      test_user_identifier: `test-user-${index.toString().padStart(3, '0')}`
    };
  ...
  generateUniversalQuestionnaireInserts(responses) {
    ...
    for (let i = 0; i < responses.length; i += 20) {
      ...
      const values = chunk.map(resp => 
        `('${resp.questionnaire_id}', NULL, '${resp.response_data.replace(/'/g, "''")}', '${resp.submitted_at}', '${resp.ip_address}', '${resp.user_agent}', '${resp.created_at}', '${resp.updated_at}')`
      ).join(',\n  ');
      
      chunks.push(`INSERT INTO universal_questionnaire_responses (questionnaire_id, user_id, response_data, submitted_at, ip_address, user_agent, created_at, updated_at) VALUES\n  ${values};`);
    }
    return chunks;
  }
...
// 主生成函数
function generateFixedTestData(config = { responseCount: 100 }) {
  console.log('🔧 生成修正版测试数据...');
  ...
  for (let i = 1; i <= config.responseCount; i++) {
    // 生成universal_questionnaire_responses数据 (不依赖外键)
    const universalResp = questionnaireGen.generateUniversalResponse(i);
    universalResponses.push(universalResp);
    
    // 生成对应的analytics_responses数据
    const analyticsResp = questionnaireGen.generateAnalyticsResponse(universalResp, i);
    analyticsResponses.push(analyticsResp);
  }
  
  // 生成SQL文件
  console.log('📁 生成SQL文件...');
  const outputDir = path.join(__dirname, '../test-data/fixed-sql');
...
`;
  ...
  universalSQLs.forEach((sql, index) => {
    fs.writeFileSync(path.join(outputDir, `02-questionnaire-batch-${(index + 1).toString().padStart(2, '0')}.sql`), sql);
  });
  
  // 分析数据
  const analyticsSQLs = sqlGen.generateAnalyticsInserts(analyticsResponses);
  analyticsSQLs.forEach((sql, index) => {
    fs.writeFileSync(path.join(outputDir, `03-analytics-batch-${(index + 1).toString().padStart(2, '0')}.sql`), sql);
  });
...
if (require.main === module) {
  generateFixedTestData({ responseCount: 100 });
}

module.exports = { generateFixedTestData };
