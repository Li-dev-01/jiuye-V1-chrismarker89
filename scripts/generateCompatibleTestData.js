...
function generateRandomId(prefix = 'test') {
...
class SQLGenerator {
  ...
  generateUniversalQuestionnaireInserts(responses) {
    ...
    for (let i = 0; i < responses.length; i += 50) {
      const chunk = responses.slice(i, i + 50);
...
SQL_DIR="test-data/compatible-sql"

echo "🗄️ 开始导入兼容测试数据到数据库: \$DATABASE_NAME"

# 1. 清理现有测试数据
echo "🧹 清理现有测试数据..."
wrangler d1 execute "\$DATABASE_NAME" --remote --file="\$SQL_DIR/01-cleanup.sql"

# 2. 导入用户数据
echo "👥 导入用户数据..."
for file in "\$SQL_DIR"/02-users-batch-*.sql; do
...
# 3. 导入问卷数据
echo "📝 导入问卷数据..."
for file in "\$SQL_DIR"/03-questionnaire-batch-*.sql; do
  echo "  执行: \$(basename "\$file")"
  wrangler d1 execute "\$DATABASE_NAME" --remote --file="\$file"
done
...
# 4. 导入分析数据
echo "📊 导入分析数据..."
for file in "\$SQL_DIR"/04-analytics-batch-*.sql; do
  echo "  执行: \$(basename "\$file")"
  wrangler d1 execute "\$DATABASE_NAME" --remote --file="\$file"
done
...
echo "✅ 数据导入完成！"
echo "📊 验证数据："
wrangler d1 execute "\$DATABASE_NAME" --remote --command="SELECT COUNT(*) as users FROM users WHERE id LIKE 'test-%';"
wrangler d1 execute "\$DATABASE_NAME" --remote --command="SELECT COUNT(*) as questionnaires FROM universal_questionnaire_responses WHERE questionnaire_id = 'employment-survey-2024';"
wrangler d1 execute "\$DATABASE_NAME" --remote --command="SELECT COUNT(*) as analytics FROM analytics_responses WHERE is_test_data = 1;"
`;
  ...
  console.log(`\n🚀 执行导入:`);
  console.log(`   bash ${path.relative(process.cwd(), path.join(outputDir, 'import.sh'))}`);
...
if (require.main === module) {
  generateCompatibleTestData({ userCount: 100, responseCount: 200 });
}
