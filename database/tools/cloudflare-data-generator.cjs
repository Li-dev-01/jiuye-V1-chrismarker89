...
/**
 * Cloudflare D1 数据库适配的故事墙测试数据生成器
 * 基于实际数据库结构生成测试数据
 */
...
class CloudflareDataGenerator {
  ...
  generateUsers(count = 50) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const user = {
        ...
        phone: Math.random() < 0.3 ? `138${String(Math.floor(Math.random() * 100000000)).padStart(8, '0')}` : null,
        nickname: isAnonymous ? this.anonymousNicknames[Math.floor(Math.random() * this.anonymousNicknames.length)] : `用户${i}`,
  ...
  /**
   * 生成SQL插入语句（适配Cloudflare D1）
   */
  generateSQL() {
    const sqlFile = path.join(this.outputDir, 'cloudflare_story_wall_data.sql');
    let sql = '-- Cloudflare D1 故事墙测试数据\n-- 生成时间: ' + new Date().toISOString() + '\n\n';
    
    // 用户数据SQL
    sql += '-- 插入用户数据\n';
    this.users.forEach(user => {
      const phone = user.phone ? `'${user.phone}'` : 'NULL';
      const nickname = user.nickname ? `'${user.nickname.replace(/'/g, "''")}'` : 'NULL';
      
      sql += `INSERT INTO users (id, email, phone, nickname, password_hash, role, is_test_data, created_at, updated_at) VALUES ('${user.id}', '${user.email}', ${phone}, ${nickname}, '${user.password_hash}', '${user.role}', ${user.is_test_data}, '${user.created_at}', '${user.updated_at}');\n`;
    });
   ...
   */
  async run() {
    console.log('🚀 开始生成Cloudflare D1适配的故事墙测试数据...\n');
...
if (require.main === module) {
  const generator = new CloudflareDataGenerator();
  generator.run();
}

module.exports = CloudflareDataGenerator;
