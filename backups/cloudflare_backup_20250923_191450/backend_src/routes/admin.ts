...
export function createAdminRoutes() {
  ...
  admin.post('/users/batch',
    ...
    async (c) => {
    try {
      const { userIds, action, data } = c.get('validatedBody');
  ...
  admin.post('/ip-access-control/rules',
    ...
    async (c) => {
    try {
      const { name, type, ipRange, description, enabled = true, priority = 1 } = c.get('validatedBody');
  ...
  admin.put('/ip-access-control/rules/:ruleId',
    ...
    try {
      ...
      const { name, type, ipRange, description, enabled, priority } = c.get('validatedBody');
