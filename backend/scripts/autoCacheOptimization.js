...
export class AutoCacheOptimizer {
    ...
    async runAutoOptimization() {
        ...
        try {
            ...
            // 3. 筛选需要应用的建议
            const applicableRecommendations = this.filterApplicableRecommendations(allRecommendations, patterns);
     ...
     * 筛选可应用的建议
     */
    filterApplicableRecommendations(recommendations, patterns) {
        return recommendations.filter(rec => {
            // 只应用高优先级和中优先级的建议
            if (rec.priority === 'low')
                return false;
            // 检查相关端点的使用模式
            const relatedPatterns = patterns.filter(p => p.endpoint.includes('statistics') || p.endpoint.includes('analytics'));
            ...
            // 应用阈值检查
            const shouldOptimize = avgPattern.cacheHitRate < this.config.applyThreshold.minCacheHitRate ||
                avgPattern.averageResponseTime > this.config.applyThreshold.maxResponseTime;
            const hasEnoughTraffic = avgPattern.requestFrequency >= this.config.applyThreshold.minRequestFrequency;
     ...
     * 应用安全限制
     */
    applySafetyLimits(recommendation) {
        ...
        if (recommendation.recommendedFrequency < safetyLimits.maxSyncFrequency ||
            ...
            // 调整到安全范围内
            recommendation.recommendedFrequency = Math.max(safetyLimits.maxSyncFrequency, Math.min(safetyLimits.minSyncFrequency, recommendation.recommendedFrequency));
        }
     ...
     * 记录优化历史
     */
    async recordOptimizationHistory(recommendation) {
        try {
      ...
      `).bind(recommendation.syncConfigId, recommendation.currentFrequency, recommendation.recommendedFrequency, recommendation.reason, JSON.stringify(recommendation.expectedImprovement)).run();
        }
        catch (error) {
     ...
     * 更新优化状态
     */
    async updateOptimizationStatus(result) {
        try {
            await this.db.prepare(`
      ...
      `).bind(result.success ? 1 : 0, result.recommendations.length, result.applied.length, result.skipped.length, result.errors.length).run();
        }
        catch (error) {
...
//# sourceMappingURL=autoCacheOptimization.js.map