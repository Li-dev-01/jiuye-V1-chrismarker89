...
export const LikeDislikeDownload: React.FC<LikeDislikeDownloadProps> = ({
  ...
  const handleLike = async () => {
    ...
    setLikeLoading(true);
    try {
      const apiEndpoint = contentType === 'heart_voice'
        ? `/api/heart-voices/${contentId}/like`
        : `/api/stories/${contentId}/like`;
  ...
  const handleDislike = async () => {
    ...
    try {
      const apiEndpoint = contentType === 'heart_voice'
        ? `/api/heart-voices/${contentId}/dislike`
        : `/api/stories/${contentId}/dislike`;
  ...
  const handleDownload = async () => {
    ...
    try {
      const apiEndpoint = contentType === 'heart_voice'
        ? `/api/heart-voices/${contentId}/png/${theme}`
        : `/api/stories/${contentId}/png/${theme}`;
  ...
  const generateAndDownload = async () => {
    try {
      ...
      const generateResponse = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/auto-png/trigger/${contentType === 'heart_voice' ? 'heart-voice' : 'story'}/${contentId}`,
        { method: 'POST' }
      );
