...
export const enhancedIntelligentQuestionnaire: UniversalQuestionnaire = {
  ...
  sections: [
    ...
    // 第3部分：在职人员详细信息（条件显示）
    {
      ...
      condition: {
        dependsOn: 'primary-identity',
        operator: 'equals',
        value: 'employed-worker'
      },
      metadata: {
        estimatedTime: 120,
        cognitiveLoad: 'medium',
      ...
      questions: [
        ...
        },
        {
          ...
          type: 'radio',
          title: '您的工作年限是？',
          description: '用于分析不同资历的薪酬水平',
          required: true,
          options: [
            { value: 'fresh', label: '应届生/1年以内' },
            { value: '1-3years', label: '1-3年' },
            { value: '3-5years', label: '3-5年' },
            { value: '5-8years', label: '5-8年' },
            { value: '8-15years', label: '8-15年' },
            { value: 'over-15years', label: '15年以上' }
          ],
          statistics: {
            enabled: true,
            chartType: 'bar',
        ...
        },
        {
          ...
          type: 'radio',
          title: '您目前的职位层级是？',
          description: '用于分析不同职位层级的薪酬分布',
          required: true,
          options: [
            { value: 'entry', label: '基层员工/专员' },
            { value: 'senior', label: '资深员工/高级专员' },
            { value: 'supervisor', label: '主管/组长' },
            { value: 'manager', label: '经理/部门负责人' },
            { value: 'director', label: '总监/高级经理' },
            { value: 'executive', label: '副总裁及以上' }
          ],
          statistics: {
            enabled: true,
            chartType: 'pie',
        ...
        },
        {
          ...
          type: 'radio',
          title: '您对目前工作的整体满意度是？',
          description: '包括薪资、发展空间、工作环境等综合考虑',
          required: true,
          options: [
    ...
    // 第4部分：学生专属信息（条件显示）
    {
      ...
      questions: [
        ...
        },
        {
          ...
          type: 'radio',
          title: '您的专业属于哪个学科门类？',
          description: '请选择您专业所属的主要学科分类',
          required: true,
          options: [
            { value: 'engineering', label: '工学（计算机、机械、电子等）' },
            { value: 'science', label: '理学（数学、物理、化学等）' },
            { value: 'medicine', label: '医学（临床医学、护理等）' },
            { value: 'economics', label: '经济学（经济、金融等）' },
            { value: 'management', label: '管理学（工商管理、公共管理等）' },
            { value: 'law', label: '法学（法律、政治学等）' },
            { value: 'education', label: '教育学（师范、体育等）' },
            { value: 'literature', label: '文学（中文、外语、新闻等）' },
            { value: 'art', label: '艺术学（美术、音乐、设计等）' },
            { value: 'agriculture', label: '农学（农业、林业等）' },
    ...
    // 第5部分：求职人员详细信息（条件显示）
    {
      ...
      condition: {
        dependsOn: 'primary-identity',
        operator: 'in',
        value: ['unemployed-person', 'recent-graduate']
      },
      metadata: {
        estimatedTime: 120,
        cognitiveLoad: 'medium',
      ...
      questions: [
        ...
        },
        {
          ...
          required: true,
          options: [
            { value: 'no-experience', label: '无正式工作经验' },
            { value: 'less-1year', label: '不到1年' },
            { value: '1-3years', label: '1-3年' },
            { value: '3-5years', label: '3-5年' },
    ...
    // 第6部分：求职详情（多维度条件：失业+求职）
    {
      id: 'job-seeking-details',
      title: '求职详情',
      description: '请分享您的求职经历和遇到的挑战',
      condition: {
        dependsOn: ['primary-identity', 'current-activity'],
        operator: 'and',
        conditions: [
          { field: 'primary-identity', operator: 'equals', value: 'unemployed-person' },
          { field: 'current-activity', operator: 'equals', value: 'job-seeking' }
        ]
      },
      metadata: {
        estimatedTime: 90,
    ...
    // 第10部分：快捷注册（条件显示）
    {
      ...
      title: '快捷注册',
      description: '创建您的半匿名身份，只需要一个昵称即可',
      condition: {
        dependsOn: 'submission-type',
        operator: 'equals',
        value: 'quick-register'
      },
      metadata: {
        estimatedTime: 30,
        cognitiveLoad: 'low',
        targetAudience: 'quick-register-users'
      },
      questions: [
        {
          ...
          type: 'text',
          title: '请设置一个昵称',
          description: '这个昵称仅用于您个人识别，不会公开显示',
          required: true,
          placeholder: '例如：求职小白、职场新人、资深码农...',
          config: {
            maxLength: 20,
            minLength: 2
          },
